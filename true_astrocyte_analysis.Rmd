...
```{r}
astro_data@meta.data
aggr_counts <- AggregateExpression(
  astro_data,
  group.by = c("sample_id", "AD_STATUS"),
  return.seurat = TRUE
)
#return.seurat is not needed but given that I already have the sample data, it stops me from having to reconstruct it. Also rember you need to pass in the taw counts

counts <- LayerData(aggr_counts, assay = "RNA", layer = "counts")
col_data <- aggr_counts@meta.data
col_data$AD_STATUS <- factor(col_data$AD_STATUS, levels = c("WT", "AD"))
# Making a DESeq2 object
